...
InnerDropdown::InnerDropdown(QWidget *parent, const style::InnerDropdown &st)
	: RpWidget(parent), _st(st), _roundRect(st::innerDropdownRadius, _st.bg), _hideTimer([=] { hideAnimated(); }),
	  _scroll(this, _st.scroll) {
	_scroll->scrolls() | rpl::on_next([=] { scrolled(); }, lifetime());

	hide();

	shownValue() | rpl::filter([](bool shown) { return shown; }) | rpl::take(1) |
		rpl::map(
			[=]
			{
		...
		rpl::flatten_latest() | rpl::filter([=] { return !isHidden(); }) |
		rpl::on_next([=] { leaveEvent(nullptr); }, lifetime());
}

QPointer<RpWidget> InnerDropdown::doSetOwnedWidget(object_ptr<RpWidget> widget) {
	auto result = QPointer<RpWidget>(widget);
	widget->heightValue() | rpl::skip(1) | rpl::on_next([=] { resizeToContent(); }, widget->lifetime());
...
void InnerDropdown::setOrigin(PanelAnimation::Origin origin) { _origin = origin; }
...
void InnerDropdown::startOpacityAnimation(bool hiding) {
	...
	_hiding = hiding;
	hideChildren();
	_a_opacity.start([=] { opacityAnimationCallback(); }, _hiding ? 1. : 0., _hiding ? 0. : 1., _st.duration);
}
...
void InnerDropdown::startShowAnimation() {
	...
	if (!_a_show.animating()) {
		...
		_showAnimation->setFinalImage(
			std::move(cache), QRect(inner.topLeft() * pixelRatio, inner.size() * pixelRatio), st::innerDropdownRadius);
		_showAnimation->setCornerMasks(Images::CornersMask(st::innerDropdownRadius));
...
void InnerDropdown::showAnimationCallback() { update(); }
...
InnerDropdown::Container::Container(QWidget *parent, object_ptr<RpWidget> child, const style::InnerDropdown &st)
	: RpWidget(parent), _child(std::move(child)), _st(st) {
	_child->setParent(this);
	_child->moveToLeft(_st.scrollPadding.left(), _st.scrollPadding.top());
}

void InnerDropdown::Container::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
